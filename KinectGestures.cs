...
//        MoveToLeft,
	...
	public virtual int[] GetNeededJointIndexes(KinectManager manager)
	{
		...
		int[] neededJointIndexes = {
			HeadIndex,leftHandIndex, rightHandIndex, leftElbowIndex, rightElbowIndex, leftShoulderIndex, rightShoulderIndex,
...
//        Debug.Log(" y " + jointsPos[hipCenterIndex].y + " x " + jointsPos[hipCenterIndex].x );
                ...
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1
                        if (jointsTracked[leftElbowIndex] && jointsTracked[leftHandIndex] && jointsTracked[leftShoulderIndex] &&
                           (jointsPos[leftElbowIndex].y - jointsPos[leftHandIndex].y) < 0.1f &&
                           (jointsPos[leftElbowIndex].y - jointsPos[leftHandIndex].y) > 0 &&
                           (jointsPos[leftShoulderIndex].y - jointsPos[leftElbowIndex].y) < 0.1f &&
                           (jointsPos[leftShoulderIndex].y - jointsPos[leftElbowIndex].y) > 0 
                          )
                        {
                            SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
                            gestureData.progress = 0.3f;
                        }
                        else if (jointsTracked[rightElbowIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] &&
                           (jointsPos[rightElbowIndex].y - jointsPos[rightHandIndex].y) < 0.1f &&
                           (jointsPos[rightElbowIndex].y - jointsPos[rightHandIndex].y) > 0 &&
                           (jointsPos[rightShoulderIndex].y - jointsPos[rightElbowIndex].y) < 0.1f &&
                           (jointsPos[rightShoulderIndex].y - jointsPos[rightElbowIndex].y) > 0)
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                            gestureData.progress = 0.3f;
                        }
                    ...
                    case 1:  // gesture - phase 2
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            bool isInPose = gestureData.joint == leftHandIndex ?
                                ...
                                (jointsPos[leftElbowIndex].y - jointsPos[leftShoulderIndex].y) < 0.1f &&
                                (jointsPos[leftElbowIndex].y - jointsPos[leftShoulderIndex].y) > 0 :
                                ...
                                (jointsPos[rightElbowIndex].y - jointsPos[rightShoulderIndex].y) < 0.1f &&
                    ...
                    case 2:  // gesture phase 3 = complete
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                           ...
                           (jointsPos[leftShoulderIndex].y - jointsPos[leftElbowIndex].y) < 0.1f &&
                           (jointsPos[leftShoulderIndex].y - jointsPos[leftElbowIndex].y) > 0 :
                                jointsTracked[rightElbowIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] &&
                           (jointsPos[rightElbowIndex].y - jointsPos[rightHandIndex].y) < 0.1f &&
                           (jointsPos[rightElbowIndex].y - jointsPos[rightHandIndex].y) > 0 &&
                           (jointsPos[rightShoulderIndex].y - jointsPos[rightElbowIndex].y) < 0.1f &&
                           (jointsPos[rightShoulderIndex].y - jointsPos[rightElbowIndex].y) > 0;
...
//							}
            ...
            // check for RightTiltHead
            case Gestures.RightTiltHead:
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1  (head is near of the right shoulder, means tilting head right)
                        if (jointsTracked[rightShoulderIndex] && jointsTracked[HeadIndex] && jointsTracked[leftShoulderIndex] &&
                            (jointsPos[rightShoulderIndex].x - jointsPos[HeadIndex].x) <0.1f 
                            &&
                           (jointsPos[HeadIndex].x - jointsPos[leftShoulderIndex].x) >0.15f
                            )
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightShoulderIndex, jointsPos[rightShoulderIndex]);
                            gestureData.progress = 0.3f;
                    ...
                    case 1:  // gesture phase 2 = complete
                        if ((timestamp - gestureData.timestamp) < 0.5f)
                        {
                            // check if head is still near of the right shoulder, means tilting head right
                            bool isInPose = jointsTracked[rightShoulderIndex] && jointsTracked[HeadIndex] && jointsTracked[leftShoulderIndex] &&
                                 (jointsPos[rightShoulderIndex].x - jointsPos[HeadIndex].x) < 0.1f &&
                            (jointsPos[HeadIndex].x - jointsPos[leftShoulderIndex].x) > 0.15f
                            ;
            ...
            // check for LeftTiltHead
            case Gestures.LeftTiltHead:
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1  (head is near of the left shoulder, means tilting head left)
                        if (jointsTracked[leftShoulderIndex] && jointsTracked[HeadIndex] && jointsTracked[rightShoulderIndex] &&
                            (jointsPos[HeadIndex].x - jointsPos[leftShoulderIndex].x) < 0.1f
                            &&
                           (jointsPos[rightShoulderIndex].x - jointsPos[HeadIndex].x) > 0.15f
                            )
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightShoulderIndex, jointsPos[rightShoulderIndex]);
                            gestureData.progress = 0.3f;
...
/*            // check for MoveToLeft
            case Gestures.MoveToLeft:
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1
                        if (jointsTracked[hipCenterIndex] &&
                           ( jointsPos[hipCenterIndex].x  >-1f )&&(jointsPos[hipCenterIndex].x  <0.7f) 
                          )
                        {
                            SetGestureJoint(ref gestureData, timestamp, hipCenterIndex, jointsPos[hipCenterIndex]);
                            gestureData.progress = 0.3f;
                    ...
                    case 1:  // gesture - phase 2
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            bool isInPose = gestureData.joint == hipCenterIndex ?
                               jointsTracked[hipCenterIndex] &&
                               (jointsPos[hipCenterIndex].x - gestureData.jointPos.x) < 0.1f :
                               jointsTracked[hipCenterIndex] &&
                               Mathf.Abs(jointsPos[hipCenterIndex].y - gestureData.jointPos.y) < 0.05f;
                ...
                break;
                */
            // check for MoveToRight
            case Gestures.MoveToRight:
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1
                        if (jointsTracked[hipCenterIndex] &&
